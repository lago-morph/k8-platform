...
| Rollout scrub | 26 pre-existing lint findings triaged (audit-before-enforce, SPEC-B5 §12): 18 noqa provenance markers (13 `ai/handoff.md`, 5 `docs/open-issues.md`); 1 noqa synthetic-constant marker (`test_live_evidence_gate.sh`); 3 findings removed by rewording account-derived literals out of forward-looking text (open-issues next-step command ×2, kyverno-lint comment ×1); 1 file-scoped exemption (`tests/live/checks/after/route53-record-live.sh`, comment example — the file sits on the fail-closed live-evidence-gated surface and editing it demands live evidence the substrate can't yet produce; an initial comment edit there turned the PR's live-evidence gate RED and was reverted; removal trigger tracked in LESSONS S7); 3 false positives (UUID tails in wait-for-claim fixtures) eliminated by the classifier's UUID handling |
