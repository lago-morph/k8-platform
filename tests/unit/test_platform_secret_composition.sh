...
RES_COUNT=$(yq -r '.spec.resources | length' "$COMP")
assert_eq "composition_resource_count" "2" "$RES_COUNT"

ASM_BASE_API=$(yq -r '.spec.resources[] | select(.name == "asm-secret") | .base.apiVersion' "$COMP")
ASM_BASE_KIND=$(yq -r '.spec.resources[] | select(.name == "asm-secret") | .base.kind' "$COMP")
ES_BASE_API=$(yq -r '.spec.resources[] | select(.name == "external-secret") | .base.apiVersion' "$COMP")
ES_BASE_KIND=$(yq -r '.spec.resources[] | select(.name == "external-secret") | .base.kind' "$COMP")
...
ASM_NAME_FMT=$(yq -r '.spec.resources[] | select(.name == "asm-secret") | .patches[] | select(.toFieldPath == "spec.forProvider.name") | .transforms[0].string.fmt' "$COMP")
...
ASM_RECOVERY=$(yq -r '.spec.resources[] | select(.name == "asm-secret") | .base.spec.forProvider.recoveryWindowInDays' "$COMP")
assert_eq "composition_asm_recovery_window_zero" "0" "$ASM_RECOVERY"

# ---- 5. ASM secret deletionPolicy is Delete (not Orphan) -----------------
ASM_DELETION=$(yq -r '.spec.resources[] | select(.name == "asm-secret") | .base.spec.deletionPolicy' "$COMP")
...
ES_NS_FROM=$(yq -r '.spec.resources[] | select(.name == "external-secret") | .patches[] | select(.toFieldPath == "metadata.namespace") | .fromFieldPath' "$COMP")
...
# itself.
ES_STORE_NAME=$(yq -r '.spec.resources[] | select(.name == "external-secret") | .base.spec.secretStoreRef.name' "$COMP")
ES_STORE_KIND=$(yq -r '.spec.resources[] | select(.name == "external-secret") | .base.spec.secretStoreRef.kind' "$COMP")
...
ES_RI_FROM=$(yq -r '.spec.resources[] | select(.name == "external-secret") | .patches[] | select(.toFieldPath == "spec.refreshInterval") | .fromFieldPath' "$COMP")
...
ASM_RC=$(yq -r '.spec.resources[] | select(.name == "asm-secret") | .readinessChecks | length' "$COMP")
ES_RC=$(yq -r '.spec.resources[] | select(.name == "external-secret") | .readinessChecks | length' "$COMP")
