...
assert_eq "terraform/management/** edit ⇒ verify-only" verify-only "$(printf 'terraform/management/helm.tf\n' | "$PROF" -)"
assert_eq "tests/live/** edit ⇒ verify-only" verify-only "$(printf 'tests/live/checks/after/foo.sh\n' | "$PROF" -)"
assert_eq "docs-only edit ⇒ none (gate not applicable)" none "$(printf 'README.md\nai/handoff.md\ndocs/site/index.md\n' | "$PROF" -)"
assert_eq "gate-machinery edit ⇒ none (circularity; unit tests + review own it)" none "$(printf '.github/scripts/required-profile-for-changes.sh\n.github/workflows/live-evidence-verify.yml\n' | "$PROF" -)"
assert_eq "mixed (incl crossplane) ⇒ full" full     "$(printf 'README.md\ncrossplane/xrds/x.yaml\n' | "$PROF" -)"
assert_eq "mixed (docs + management) ⇒ verify-only" verify-only "$(printf 'docs/site/index.md\nterraform/management/main.tf\n' | "$PROF" -)"
