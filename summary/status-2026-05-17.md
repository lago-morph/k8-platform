...
|---|-----------|--------|----------|-------|
| 0 | Base environment (VPC, Route53, Cognito, ACM) | **Shipped** | `terraform/base/` (8 `.tf` files + `terraform.tfvars.example`, ~427 LoC of HCL), apply+destroy confirmed 2026-05-04 (handoff.md) | Cognito groups still need to be added before Iter 5 begins (see ADR-007 consequence) |
| 1 | Management cluster (EKS, IRSA, ArgoCD, Crossplane v2, ESO) | **Plan-green, NOT applied** | `terraform/management/` (8 `.tf` files + `terraform.tfvars.example`, ~681 LoC HCL); PRs #3, #7, #9 all merged; E2E verification scaffolded in CI but never observed passing | Crossplane upgraded to v2 APIs in PR #7 but the new manifest application path is untested live |
...
| **1 Mgmt cluster** | EKS module config (v20+), IRSA roles for ArgoCD/Crossplane/ESO/ExternalDNS (4 roles — `irsa.tf` explicitly notes "cert-manager is not installed on the management cluster; ACM handles TLS"), Helm installs (ArgoCD, Crossplane v2.0.1, ESO, ingress-nginx — 4 `helm_release` resources; ExternalDNS has an IRSA role but no Helm release yet), ArgoCD ingress via Helm values, `terraform_data` + `local-exec` to apply Crossplane manifests, E2E check steps in CI workflow covering namespaces `argocd`, `crossplane-system`, `external-secrets`, `ingress-nginx` | A **single successful `apply-and-destroy` run** with all E2E assertions green; verify ArgoCD UI reachable; verify Crossplane `DeploymentRuntimeConfig` timing |
...
### PRs merged over time (all 10 merged)
...
Notes:
- All 10 PRs (#1, #3–#11) are merged; #2 was an issue, not a PR.
- PRs #4 and #5 both merged on `chore/docs-handoff-and-runbook` within minutes; #5 added the `logs/` directory on top of #4.
...
|-----------|--------------------------|----------------|----------|
| 0 Base | initial scaffold via direct commits to main (pre-PR #1: `3c08966` "Bootstrap repo structure and scaffold Iteration 0"); then #1 added CI; #3, #6 fixed plan/E2E issues; #9 added base E2E checks | Add Cognito groups for Iter 5 | None |
| 1 Mgmt cluster | initial scaffold via direct commit `3765c29`; then #3 (validate), #7 (Crossplane v2 + Helm ingress), #9 (mgmt E2E) | Live apply-and-destroy validation | Pluralsight sandbox session must be live; risks: Crossplane CRD timing, ArgoCD ingress DNS |
...
| B6 | **Cognito groups `k8s-admins`, `k8s-viewers` not yet added to `terraform/base/`** (handoff `What Was Done — 2026-05-10`, "Immediate next step" sub-bullet). | Iter 5 cannot be tested end-to-end without them. | Add to `terraform/base/cognito.tf` whenever Iter 1 apply has been validated (so a base re-apply is cheap), or just before Iter 5 starts. |
...
| F7 | **`docs/operations.md` references `ai/testing-overview.md`** (under "Where to Find Things"), but that file was moved to `ai/archive/` in PR #8 (2026-05-04) cleanup. | Stale link discovered by reading `docs/operations.md` §"Where to Find Things". | High (already true) | Low (cosmetic) | Update the link to `ai/archive/testing-overview.md` or replace with a pointer to the new skill. |
...
```
terraform/base/          8 .tf files (+tfvars.example), ~427 LoC HCL   SHIPPED
terraform/management/    8 .tf files (+tfvars.example), ~681 LoC HCL   PLAN-GREEN
...
.claude/skills/          8 skills present (2 repo-specific, 6 from factory)
```
...
2. **Session N+1:** Add Cognito groups to `terraform/base/cognito.tf` (B6); re-plan to confirm. Open GitHub issues for the Iter 2 deliverables (REQ-XP-01..04 plus the ArgoCD `Application` and end-to-end claim test — ~5 items) (closes B5 partially).
